  ...
  fNEvents(nEvents)
{
  ...
  fPtPi = new TH1F("PtPi", "Transverse momentum pi+ pi- Pair", 100, 0, 0.25);
  fPtKa = new TH1F("PtKa", "STARlight Coherent K^{+} K^{-} Pair P_{T}; P_{T} (GeV/c); counts", 100, 0, 0.3);
  fPtKa_tpc = new TH1F("PtKa_tpc", "Transverse momentum k+/k-; P_{T} (GeV/c); counts", 100, 0, 0.3);
  ...
  fEtaPi = new TH1F("EtaPi", "Eta pi+/pi-", 200, -8., 8.);
  fEtaEl = new TH1F("EtaEl", "Eta e+/e-", 200, -8., 8.);

  fInvMassEl = new TH1F("InvMassEl", "Invariant mass", 100, 0.02, 0.6);
  fInvMassMu = new TH1F("InvMassMu", "Invariant mass", 100, 0, 5);
  fInvMassPi = new TH1F("InvMassPi", "Invariant mass", 100, 0.3, 1.6);
  fInvMassKa = new TH1F("InvMassKa", "Invariant mass", 100, 0.99, 1.05);
  ...
  fEta1 = new TH1F("fEta1", "Eta track 1", 200, -8, 8);
  fEta2 = new TH1F("fEta2", "Eta track 2", 200, -8, 8);
}
...
void Analyze::doAnalysis()
{
  ...
  for(Int_t ev = 0; ev < fNEvents; ev++){
    ...
    fEta1->Fill(vecArr[0]->PseudoRapidity());
    fEta2->Fill(vecArr[1]->PseudoRapidity());
    ...
    if(idpart== 11 || idpart== -11){
      ...
      fEtaEl->Fill(sum.PseudoRapidity());
      ...
      fEtaPi->Fill(sum.PseudoRapidity());
      ...
      fEtaKa->Fill(sum.PseudoRapidity());
