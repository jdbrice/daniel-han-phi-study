...
int main(int argc, char **argv) {
  ...
  TH1F *combined_masses =
      new TH1F("Combined Masses",
               "Toy Model Combined Masses;m_{K^+ K^-}(GeV);count", 300, 0.8, 3.);

  Selector pid = Selector();

  for (int i = 0; i < parent_vector.size(); i++) {
    if (pid.get_NSigmaKaon(daughter1_vector[i]) < 5. &&
        pid.get_NSigmaPion(daughter1_vector[i]) > 5. &&
        pid.get_NSigmaKaon(daughter2_vector[i]) < 5 &&
        pid.get_NSigmaPion(daughter2_vector[i]) > 5.) {
