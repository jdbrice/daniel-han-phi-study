...
double PHI_SAMPLE_SIZE = 2e5;
double RHO_SAMPLE_SIZE = 4e5;
  ...
  for (int i = 0; i < RHO_SAMPLE_SIZE; i++) {
    ...
    std::vector<TLorentzVector *> daughter_ptr_pair =
        Random_routines::symmetrical_two_body_decay(rho_parent_particle_ptr,
                                                    PION_MASS);
    Random_routines::add_gaussian_pt_error(daughter_ptr_pair[0], 0.04);
    Random_routines::add_gaussian_pt_error(daughter_ptr_pair[1], 0.04);
