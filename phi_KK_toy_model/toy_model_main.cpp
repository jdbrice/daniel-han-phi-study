...
int main(int argc, char **argv) {
  ...
  TH2F *all_pt_NPion = new TH2F(
      "All Tracks", "ToyModel N#sigmaPion All Tracks;P_{T}(GeV/c);N#sigmaPion",
      100, 0., 1.2, 200, -20, 80);

  TH2F *all_pt_NKaon = new TH2F(
      "All Tracks", "ToyModel N#sigmaKaon All Tracks;P_{T}(GeV/c);N#sigmaKaon",
      100, 0., 1.2, 200, -60, 80);
  ...
  for (int i = 0; i < PHI_SAMPLE_SIZE; i++) {
    ...
    if (abs(nsigmakaon1) < 5 && abs(nsigmakaon2) < 5 && pt1 > 0.06 &&
        pt2 > 0.06) {
      kaon_pt->Fill(pt1);
      kaon_pt->Fill(pt2);
