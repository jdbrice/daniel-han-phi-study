...
void nsigma_manual_k() {
  ...
  while (myReader.Next()) {
    ...
    if (pair->mChargeSum == 0 &&
        ...
        abs(pair->d2_mNSigmaKaon) < 30 &&
        abs(pair->d1_mNSigmaKaon) < 30) {
  ...
  for (int i = 1; i <= kaon_pt_NKaon->GetNbinsX(); ++i) {
    // Projection Y for each bin in X
    TH1D *y_proj = kaon_pt_NKaon->ProjectionY("_py", i, i);
    ...
    if ((int)fit_result == 0) { // case for successful fit
      ...
      pt_values.push_back(kaon_pt_NKaon->GetXaxis()->GetBinCenter(i));
  ...
  TH1D *all_momentum = kaon_pt_NKaon->ProjectionY();
  TFitResultPtr fitResult =
      all_momentum->Fit("gaus", "QS"); // Fit the projection
  double nsigmakaon_mean = fitResult->Parameter(1);
  double nsigmakaon_error = fitResult->Error(1);
  TF1 *all_momentum_fit = all_momentum->GetFunction("gaus");

  // graphing the momentum-dependent result
  std::cout << nsigmakaon_mean << "    " << nsigmakaon_error <<  std::endl;
  // Create TGraph
  makeCan();
  ...
  TBox *meanBox =
      new TBox(pt_values.front(), nsigmakaon_mean - nsigmakaon_error,
               pt_values.back(), nsigmakaon_mean + nsigmakaon_error);

  meanBox->SetFillColor(kBlue);
  meanBox->SetFillStyle(3004);
  meanBox->Draw("same");

  TLine *meanLine = new TLine(pt_values.front(), nsigmakaon_mean,
                              pt_values.back(), nsigmakaon_mean);
