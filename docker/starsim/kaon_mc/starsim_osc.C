...
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
void starsim(Int_t nevents, Int_t jobNum, const char* dir, Int_t rngSeed = 1234) {
  TString jobDir = TString::Format("%s/%d/", dir, jobNum);
  TString slightFileName = TString::Format("slight_%d.out", jobNum);
  TString fzdFileName = TString::Format("slight_%d.fzd", jobNum);
  TString rootFileName = TString::Format("kinematics_starsim_%d.root", jobNum);
  TString slightFullPath = jobDir + slightFileName;
  TString fzdFullPath = jobDir + fzdFileName;
  TString rootFullPath = jobDir + rootFileName;
  FILE *my_file = OpenFile(slightFullPath.Data());
  gROOT->SetMacroPath(
  ...
  {
    _primary->SetFileName(rootFullPath.Data());
